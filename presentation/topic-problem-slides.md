...
1. Open `topic-problem-slides.html` in a browser → press `F` for fullscreen → use `←` `→` / Space.
2. Copy each slide section from this Markdown into PowerPoint / Google Slides (one slide per section).
